...
# 특정 서비스에 대한 *를 구체적인 액션으로 변환
def convert_asterisk_action(service):
    perList = []
    actionList = ServiceActionDict[service]
...
# 특정 서비스에 대한 ~*를 구체적인 액션으로 변환
def convert_asterisk_specific_action(service, actionPrefix):
    perList = []
    actionList = ServiceActionDict[service]
    for action in actionList:
...
# permission에서 * 를 구체적인 서비스, 액션으로 변환
def convert_service_action(permission):
    perList = []
    if permission == '*':
        for service in ServiceActionDict.keys():
            new = convert_asterisk_action(service)
            perList = perList + new
    else:
        temp = permission.split(':', 1)
        service = temp[0]
        action = temp[-1]
        if action == '*':
            perList = convert_asterisk_action(service)
        elif '*' in action:
            actionPrefix = action.split('*', 1)[0]
            perList = convert_asterisk_specific_action(service, actionPrefix)
        else:
            perList.append(permission)
    return perList

# policy에 정의된 service, permission 추출
def get_permission_from_policy_statements(statementList):
    permissionDict = {'Allow':{'Action':[], 'NotAction':[]}, 'Deny':{'Action':[], 'NotAction':[]}}
    for statement in statementList:
        effect = statement['Effect']
        keys = ['Action', 'NotAction']
        for key in keys:
            if key in statement.keys():
                if isinstance(statement[key], list):
                    for per in statement[key]:
                        per = convert_service_action(per)
                        cvt.update_dict_key_value(permissionDict[effect], key, per)
                else:
                    per = convert_service_action(statement[key])
                    cvt.update_dict_key_value(permissionDict[effect], key, per)
    for element in permissionDict.values():
        for key, per in element.items():
            element[key] = list(set(per))
    return permissionDict

# permissionDict 병합
def merge_permission(perDict1, perDict2):
    mergeDict = {}
    for effect in perDict1.keys():
        mergeDict[effect] = {}
        for key in perDict1[effect].keys():
            per1 = set(perDict1[effect][key])
            per2 = set(perDict2[effect][key])
            mergeDict[effect][key] = list(per1 | per2)
    return mergeDict

# permission 차집합(perDict1 - perDict2)
def get_diff_permission(perDict1, perDict2):
    diffPerDict = {}
    for effect, element in perDict1.items():
        diffPerDict[effect] = {}
        for key, perList in element.items():
                per1 = set(perDict1[effect][key])
                per2 = set(perDict2[effect][key])
                diffPerDict[effect][key] = list(per1 - per2)
    return diffPerDict

# permission 교집합(perDict1 & perDict2)
def get_intersection_permission(perDict1, perDict2):
    interPerDict = {}
    for effect, element in perDict1.items():
        interPerDict[effect] = {}
        for key, perList in element.items():
                per1 = set(perDict1[effect][key])
                per2 = set(perDict2[effect][key]) 
                interPerDict[effect][key] = list(per1 & per2)
    return interPerDict

# permission 비었는지 확인
def is_permission_empty(perDict):
    for effect in perDict.keys():
        for key, perList in perDict[effect].items():
            if len(perList) > 0:
                return False    # full
    return True # emtpy

# permission 같은지 확인
def is_permission_same(perDict1, perDict2):
    for effect, element in perDict1.items():
        for key, perList in element.items():
            per1 = set(perDict1[effect][key])
            per2 = set(perDict2[effect][key]) 
...
def merge_permission_if_same_resource(permissionList, resourceList, permissionDict, resource):
    if resource in resourceList:
        idx = resourceList.index(resource)
        permissionList[idx] = merge_permission(permissionList[idx], permissionDict)
    else:
        resourceList.append(resource)
        permissionList.append(permissionDict)
    return resourceList, permissionList

def get_policy_statement_info(policyKey, statementList):
    info = {}
    resourceList = []
    permissionList = []
    for statement in statementList:
        resource = statement['Resource']
        perDict = get_permission_from_policy_statements([statement])
        if isinstance(resource, list):
            for res in resource:
                resourceList, permissionList = merge_permission_if_same_resource(permissionList, resourceList, perDict, res)
        else:
            resourceList, permissionList = merge_permission_if_same_resource(permissionList, resourceList, perDict, resource)
    info[policyKey] = {'Resource': resourceList, 'Permission': permissionList}
    return info

# permission 리스트를 service-action dictionary 구조로 변환
def permission_to_service_action(perList):
    result = {}
    for permission in perList:
        temp = permission.split(':')
        service = temp[0]
        action = temp[-1]
        cvt.update_dict_key_value(result, service, action)
    # result[service] = sorted(result[service])
    return result
